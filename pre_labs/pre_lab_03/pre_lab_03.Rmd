...
**Answer** In the first 20 results, there were four issues.
...
**Answer** The amount field is numerical. The name is character. The id column is numeric.
```{r}
...
**Answer**: There are 192959 loans where the franchise name is NA. There are 275 loans wehre the fanchise name is Subway.
...
**Answer**: There are 135073 loans where the forgiveness_amount is NA.
...
**Answer**: There are 135073 loans wehre the forgivenss_amount is NA. It is the same as the question above.
...
**Answer**: There are 135073 loans, meaning that all of the loans with a forgiveness amount of NA also have a forgiveness date of NA. I can conclude from this answer that either these loans were all entered incorrectly or there has yet to be a decision on whether the loans will be forgiven and may still be going through the official process.
```{r}
ppp_maryland_loans %>%
...
**Answer** There are no loans in the data for October 2020.
```{r}
ppp_maryland_loans %>%
...
**Answer** The largest loan is for 10000000 and the smallest is for 6. I find both of these numbers odd. The 10000000 amount is the maximum allowed by the program, accoding to the SBA. I would want to verify that this was a large company or a company where it would make sense that payroll costs were so high. The loan of 6 must be statistically inaccurate. According to SBA standards, receiving a loan of that amount would mean that the company's payroll costs would be less than a dollar a month. 
```{r}
...
**Answer** All of the amounts align in the fields.
...
**Answer** It is a character datatype.
```{r}
...
**Answer** The amount field is stored as a character, which if it were run would prevent the program from being able to complete the math. The field needs to be changed and stored as a numeric field. Also, the headers need to be cleaned. NAME should be name, street address should be street_address, etc. It is easier for R to run the data if this is cleaned, and there is a simple fix that R will do by itself if the right code is run. Finally, there first two records are duplicates of each other. You can fix this by running distinct() to keep a single copy of each record.
...
**Answer** It means that the amount field is a character datatype and cannot be summarised by the program. This needs to be changed to a numeric datatype.
...
**Answer** There is one duplicate. It is Stuart Jones LLC.
...
**Task** Run the following codeblock to use str_tot_title() to standarize capitalization in the "city" field. How many mispellings of Arnold remain after running this code? Answer below. Two mispellings remain.
