...
**Answer** It shows each state and who it voted for in 2016, the percentage of the vote, the percent of businesses receiving ppe loans and the ppe amount per employee.
```{r}
...
**Answer** Scatter plot of the percent that a state voted for Trump and the percent of businesses that got the ppe loans. There is also a line of regression.
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** The same type of plot, but the output seems a bit more random with less of a coorelation
```{r}
...
**Answer** There is a stronger coorelation for the percentage of businesses receiving ppe loans than the amount per employee.
```{r}
reveal_data %>%
...
**Answer** The p value is very low, showing that the stat is somewhat significant.
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer** The P value is much higher, above the .05 threshold that we discussed in class.
```{r}
...
**Answer** It calculated the mean of amount per employee and percent of businesses receiving ppe loans for states that voted for trump or clinton in 2016.
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer** The t test is around -1 and the p value is .2295, which is way above the .05 threshold. This should give a reporter pause.
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** The p value is much much lower and the t test is just above 3. This means this finding is more statistically signifant and not random. This is reassuring.
```{r}
